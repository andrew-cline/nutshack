...
function validate_info($vitamins, $minerals, $protein, $fat, $carb, $calories)
{
    $error_messages = array(); # Create empty error_messages array.
    if (strlen($calories)  == 0) {
        array_push($error_messages, "Calories field must have a number.");
        if(!is_numeric($calories))
        {
            array_push($error_messages, "Calories must be a number");
        }
    }

    if (strlen($vitamins) == 0) {
        array_push($error_messages, "Vitamin field can not be empty.");
    }
    if (strlen($minerals) == 0) {
        array_push($error_messages, "Mineral field can not be empty.");
    }
    if (strlen($protein) == 0) {
        array_push($error_messages, "Protein field can not be empty.");
    }
    if (strlen($fat) == 0) {
        array_push($error_messages, "Fat field can not be empty.");
    }
    if (strlen($carb) == 0) {
        array_push($error_messages, "Carb field can not be empty.");
    }
